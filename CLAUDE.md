...
If several Pis share the LAN, verify the SSH host key before assuming an IP is this
device; a MAC-OUI scan alone has picked the wrong one before.
...
**It already carries USB firmware — no flash was needed.** The 2-channel/16 kHz linear
image is not redistributed here (Seeed publishes it without a licence); run
`hermit/firmware/fetch.sh` to download it and verify it against `SHA256SUMS`. The wiki's filenames (`respeaker_flex_ua-io16-lin.bin`) do not exist; the
...
The project trained its own wake word in-house (the training repo is private);
the exported classifier ships here as `hermit/models/hey_sudo.onnx` and is a
**livekit-wakeword** (openWakeWord-style) classifier. Do not go looking for a Picovoice key — it is not needed and the
...
`hermit/tools/sudo-console.py` — adapted from an earlier internal console design.
...
as another user cannot write control.json. The earlier project solved this with a sudoers entry.
For now test sessions use /tmp/hermit-state.

## Earcons (audible acks)

`hermit/assets/earcons/` — mono s16le 16 kHz chirps generated by
`scripts/make_earcons.py` (pinned by SHA256SUMS). Two are wired in
`src/speech/earcons.rs` + `gateway/voice.rs`:

- `trigger_ack.wav` — plays the INSTANT the wake word (or `/listen`) fires.
  Ordering matters: it must be enqueued AFTER `player.flush()`, because the flush
  bumps the audio generation and would discard a chirp enqueued before it.
- `thinking.wav` — plays when the utterance is finalized (end of speech), before
  the LLM round trip: "stopped listening, working on it".

The loader is a real RIFF chunk walk, not a blind 44-byte skip (LIST/INFO chunks
before `data` would otherwise play as a click). Wrong-format files are rejected
loudly — regenerate, never resample. Missing files log once and stay silent;
earcons never gate boot. Four spare earcons (`listening`, `ready`,
`session_close`, `attention`) ship unwired.
